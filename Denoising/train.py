...
if __name__ == '__main__':
    ...
    def valid(model, val_loader, epoch, writer):
        model.eval()
        psnr_epoch = 0
        ssim_epoch = 0
        ...
        writer.add_scalar('PSNR', psnr_epoch, epoch)
        writer.add_scalar('SSIM', ssim_epoch, epoch)
    ...
    eigennet = models.unet.UNet_n2n_un(in_channels=4, out_channels=3, opt=opt).cuda()
    copy_model = models.unet.UNet_n2n_un(in_channels=4, out_channels=3, opt=opt).cuda()
    ...
    for epoch in range(100):
        ...
        valid(eigennet, val_loader, epoch, writer)
