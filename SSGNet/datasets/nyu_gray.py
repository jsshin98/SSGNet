...
def knn_affinity(image, n_neighbors=[20, 10], distance_weights=[2.0, 0.1]):
    ...
    for k, distance_weight in zip(n_neighbors, distance_weights):
        f = np.stack(
            ...
            [c, distance_weight * x, distance_weight * y],
            axis=1,
...
class NYUDepthV2(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = np.array(sample[1].permute(1, 2, 0))

        img = Image.fromarray((img * 255).astype(np.uint8)).convert('L')
        img = np.array(img)

        affinity, D = KNN_laplacian(img)
        img = torchvision.transforms.ToTensor()(img)
        affinity = torch.tensor(affinity.todense()).float()
        D = torch.tensor(D.todense()).float()
